...
bool get_expected_lengths(size_t &expected_pk_len, size_t &expected_ct_len,
                          uint64_t ms)
{
#ifdef USE_LIBOQS
    OQS_KEM *kem = OQS_KEM_new("Kyber512");
...
void handle_hello(const Parsed &p, int sock)
{
    ...
    if (!p.encaps.empty())
    {
        success = handle_encaps_present(pi, p, expected_ct_len, key_context,
                                        peer_name, ms);
    }
    else if (initiator && pi.sk.key.empty() && !pi.sent_hello)
    {
        success = handle_initiator_no_encaps(pi, p, expected_pk_len,
    ...
    try
    {
        ...
        else [[likely]]
        {
            const std::string shortfp = sender_fp_hex.empty() ? "(no fp)"
                                        : sender_fp_hex.size() > 10
                                            ? sender_fp_hex.substr(0, 10)
                                            : sender_fp_hex;
    ...
    while (std::getline(std::cin, line) && is_connected)
    {
            ...
            for (const auto &token : recipients)
            {
                ...
                for (const auto &kv : peers)
                {
                    if (kv.first.size() >= token.size() &&
                        std::equal(
                            token.begin(), token.end(), kv.first.begin(),
        ...
        for (const auto &r : ready_recipients)
        {
            ...
            const secure_vector nonce =
                derive_nonce_from_session(session_key, seq);
...
try
{
    ...
    // NOLINT(clang-analyzer-deadcode.DeadStores) - port is used in connect_to()
    // call
...
#ifdef USE_LIBOQS
        const auto kp =
            load_pqsig_keypair(KeyPath{args[4]}, AlgorithmName{"ML-DSA-87"});
        if (kp.sk.empty())
