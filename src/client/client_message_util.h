...

{
  ...
  if (itset != peer_globals::fps_by_username().end() && !itset->second.empty())
...
inline std::string build_key_context_for_session(PeerFpHexIn peer_fp_hex,
  ...
  return build_key_context_for_peer(
      KeyContextParams::make(KeyContextParams::MyFP{peer_globals::my_fp_hex()},
...
inline void handle_hello(const Parsed &p) {
  ...
  if (!update_peer_state(pi, p, PeerNameIn{peer_name}, PeerKeyIn{peer_key},
                         PeerFpHexIn{peer_fp_hex}, FpsSetRef{fps_set}, MsU{ms}))
    return;
...
inline std::string compute_sender_fp_hex(const PeerInfo &pi) {
  if (pi.identity_pk.empty())
    return {};
  const auto fp = fingerprint_sha256(
      std::vector<unsigned char>(pi.identity_pk.begin(), pi.identity_pk.end()));
...
inline bool handle_sequence(PeerInfo &pi, SeqU64 seq, MsU ms,
  ...
  if (is_replay_attack(seq.v, pi.recv_seq)) [[unlikely]] {
    dev_println("[" + std::to_string(ms.v) + "] REJECTED: replay attack from " +
  ...
  if (jitter_detected) [[unlikely]] {
    dev_println("[" + std::to_string(ms.v) +
                "] WARNING: jitter detected from " + std::string(peer_from.v) +
                " got=" + std::to_string(seq.v) +
                " expected=" + std::to_string(pi.recv_seq));
    pi.recv_seq = seq.v;
  } else if (seq.v != pi.recv_seq) [[unlikely]] {
    dev_println("[" + std::to_string(ms.v) + "] REJECTED: seq mismatch from " +
                std::string(peer_from.v) + " got=" + std::to_string(seq.v) +
                " expected=" + std::to_string(pi.recv_seq));
    return false;
  }
...
inline void handle_chat(const Parsed &p) {
  ...
  if (is_message_timeout_exceeded(pi->last_recv_time, ms,
    ...
    dev_println("[" + std::to_string(ms) + "] WARNING: large time gap from " +
                peer_from);
  ...
  try {
    ...
    const auto aad = build_aad_seq(SenderFpHexIn{sender_fp_hex}, SeqU64{p.seq});
...
resolve_fingerprint_recipients(const std::vector<std::string> &recipients) {
  ...
  for (const auto &token : recipients) {
    if (!is_valid_hex_token(token)) {
      std::cout << "REJECTED: recipient must be hex fingerprint prefix (got: "
                << token << ")\n";
      continue;
    }
    ...
    if (matches.size() == 1) {
      ...
      if (fp.starts_with("uname:")) {
        std::cout << "REJECTED: cannot send to non-verified peer " << token
                  << "\n";
        continue;
      }
...
inline bool send_message_to_peer(const std::string &msg,
  ...
  const auto ctx =
      prepare_message_context(recipient_fp.value, peer_globals::my_fp_hex(),
                              peer_globals::peers(), peer_globals::peers_mtx());
  if (!ctx.valid) {
    std::cerr << "[" << get_current_timestamp_ms()
        ...
        [frame_ptr, target = ctx.target_username,
         fp = recipient_fp.value](const std::error_code &ec, std::size_t) {
          if (ec) {
            ...
            std::cerr << "[" << get_current_timestamp_ms()
                      << "] send_message_to_peer: write completed to " << target
                      << "\n";
          }
        });
  }
