    ...
    async_read_frame(
        ...
        [frame_buf, read_next](const std::error_code &ec, std::size_t) {
          ...
          try {
            ...
            Parsed p = parse_payload(span.subspan(4).data(), span.size() - 4);
            std::cerr << "[DEBUG] asio_reader_loop: received message type="
                      << int(p.type) << "\n";

            switch (p.type) {
            case MSG_HELLO:
              std::cerr << "[DEBUG] handling MSG_HELLO from " << p.username
                        << "\n";
              handle_hello(p);
              break;
            case MSG_CHAT:
            ...
            case MSG_PUBKEY_RESPONSE:
              ...
              break;
            default:
              std::cerr << "[DEBUG] unknown message type " << int(p.type)
                        << "\n";
              break;
            }
...
inline void writer_thread() {
  ...
  while (runtime_globals::is_connected().load(std::memory_order_acquire)) {
    ...
    {
      if (std::cin.eof()) {
        std::cerr << "[" << get_current_timestamp_ms() << "] stdin closed\n";

        std::cin.clear();
        runtime_globals::is_connected().store(false, std::memory_order_release);
        break;
      }

      if (std::cin.fail()) {
        std::cerr << "[" << get_current_timestamp_ms() << "] getline failed\n";
        std::cin.clear();
        continue;
      }
