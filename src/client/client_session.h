...
attempt_connection_async(asio::io_context &io, ServerStr server, PortInt port,
  ...
  async_connect_to_host_asio(
      ...
      [completion_handler](std::shared_ptr<asio::ip::tcp::socket> sock,
        ...
        auto stream = std::make_shared<ssl_socket>(std::move(*sock),
                                                   *runtime_globals::ssl_ctx());
        ...
        stream->async_handshake(
            ...
            [stream, completion_handler](const std::error_code &ec) {
              ...
              sig_data.insert(sig_data.end(), peer_globals::my_eph_pk().begin(),
              ...
              auto frame_ptr =
                  std::make_shared<std::vector<unsigned char>>(hello_frame);

              async_write_frame(stream, frame_ptr,
                                [completion_handler, frame_ptr](
