...
int main(int argc, char **argv)
{
    ...
    while (true)
    {
        ...
        for (int c : clients)
        {
            ...
            ssize_t       n = recv(c, lenbuf, 4, MSG_PEEK);
            if (n < 0)
            {
            ...
            ssize_t                    got = full_recv(c, buf.data(), 4 + len);
            if (got <= 0)
            {
                to_remove.push_back(c);
                continue;
            }
            uint32_t payload_len = read_u32_be(buf.data());
            try
            {
                Parsed p = parse_payload(buf.data() + 4, payload_len);
                if (p.type == MSG_HELLO)
                {
                    ...
                    {
                        sid = std::string(
                            reinterpret_cast<const char *>(p.eph_pk.data()),
                            p.eph_pk.size());
                    }

                    if (p.id_alg == 0 || p.identity_pk.empty() ||
                        p.signature.empty())
                    {
                        std::cout
                            << "REJECTED: missing identity or signature for "
                            << uname << "\n";
                        to_remove.push_back(c);
                        continue;
                    }
                    ...
                    sig_data.insert(sig_data.end(), p.eph_pk.begin(),
                                    p.eph_pk.end());
                    sig_data.insert(sig_data.end(), p.session_id.begin(),
                                    p.session_id.end());

                    bool sig_ok = false;
                    if (p.id_alg == ALGO_MLDSA87)
                    {
                        sig_ok = pqsig_verify("ML-DSA-87", p.identity_pk,
                                              sig_data, p.signature);
                    }
                    else
                    {
                        sig_ok = false;
                    ...
                    if (it_existing != sd.fd_by_nick.end() &&
                        it_existing->second != c)
                    {
                        std::cout << "REJECTED: username already in use "
                                  << uname << "\n";
                        to_remove.push_back(c);
                        continue;
                    }
                    ...
                    if (too_similar_username(uname, sd.fd_by_nick) &&
                        (it_existing == sd.fd_by_nick.end() ||
                         it_existing->second != c))
                    {
                        std::cout << "REJECTED: username too similar to "
                                     "existing user "
                                  << uname << "\n";
                        to_remove.push_back(c);
                        continue;
                    ...
                    auto it_oldnick = sd.nick_by_fd.find(c);
                    if (it_oldnick != sd.nick_by_fd.end() &&
                        it_oldnick->second != uname)
                    {
                        std::string oldnick = it_oldnick->second;
                        sd.fd_by_nick.erase(oldnick);
                        auto it_fp_existing = sd.fingerprint_hex_by_fd.find(c);
                    ...
                    sd.nick_by_fd[c]     = uname;
                    sd.fd_by_nick[uname] = c;
                    if (!fp_hex.empty())
                    {
                        sd.fingerprint_hex_by_fd[c]           = fp_hex;
                        sd.fd_by_fingerprint[fp_hex]          = c;
                    ...
                    for (auto &kv : sd.fd_by_nick)
                    {
                        ...
                        if (fd == c)
                            continue;
                        full_send(fd, buf.data(), buf.size());
                    }
                    ...
                    for (auto &kv : sd.hello_message_by_fingerprint)
                    {
                        ...
                        if (!existing_fp.empty())
                        {
                            ...
                            if (itn != sd.nick_by_fingerprint.end() &&
                                itn->second == uname)
                                continue;
                        }
                        ...
                        try
                        {
                            uint32_t existing_payload_len =
                                read_u32_be(existing_hello.data());
                            Parsed p2 = parse_payload(existing_hello.data() + 4,
                            ...
                            full_send(c, stripped.data(), stripped.size());
                        }
                        catch (...)
                        {
                            full_send(c, existing_hello.data(),
                                      existing_hello.size());
                        }
                    }
                }
                ...
                else if (p.type == MSG_CHAT)
                {
                    ...
                    if (dst != -1)
                        full_send(dst, buf.data(), buf.size());
                }
                else if (p.type == MSG_LIST_REQUEST)
                {
                    ...
                    full_send(c, resp.data(), resp.size());
                }
                else if (p.type == MSG_PUBKEY_REQUEST)
                {
                    ...
                    if (itn != sd.fd_by_nick.end())
                    {
                        ...
                        {
                            auto itpk = sd.identity_pk_by_fingerprint.find(
                                itfp->second);
