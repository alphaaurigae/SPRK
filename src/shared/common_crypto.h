...
public:
    ...
    [[nodiscard]] const unsigned char* data() const noexcept { return data_.data(); }
    [[nodiscard]] constexpr std::size_t size() const noexcept { return N; }
    [[nodiscard]] auto begin() noexcept { return data_.begin(); }
    [[nodiscard]] auto end() noexcept { return data_.end(); }
    [[nodiscard]] unsigned char& operator[](std::size_t i) noexcept { return data_[i]; }
    [[nodiscard]] const unsigned char& operator[](std::size_t i) const noexcept { return data_[i]; }
...
#else
    throw std::runtime_error("pqkem not enabled at build for " + std::string(oqs_alg));
#endif
}

inline std::pair<std::vector<unsigned char>, secure_vector>
pqkem_encaps(std::string_view oqs_alg,
             std::span<const unsigned char> peer_pk)
{
#ifdef USE_LIBOQS
    OQS_KEM_ptr kem(OQS_KEM_new(std::string(oqs_alg).c_str()));
    if (kem == nullptr)
...
pqsig_sign(std::string_view oqs_alg,
           SecretKey sk,
           Message msg)
{
#ifdef USE_LIBOQS
    ...
    if (OQS_SIG_sign(sig.get(), out.data(), &siglen, msg.data.data(), msg.data.size(),
                     sk.data.data()) != OQS_SUCCESS)
    {
                         ...
                         Signature signature)
{
    ...
    const bool ok = (OQS_SIG_verify(sig.get(), msg.data.data(), msg.data.size(),
...
{
    return pqsig_verify(oqs_alg, PublicKey{pk}, Message{msg}, Signature{signature});
}
...
inline std::vector<unsigned char>
aead_encrypt(const secure_vector& key,
             ...
             std::vector<unsigned char>& out_nonce)
{
    ...
    if (1 != EVP_EncryptInit_ex(ctx.get(), EVP_chacha20_poly1305(), nullptr, nullptr,
                                nullptr))
    {
    ...
    if (1 != EVP_EncryptFinal_ex(ctx.get(), ciphertext_span.subspan(clen).data(), &len))
    {
...
inline std::vector<unsigned char>
aead_encrypt_with_nonce(const secure_vector& key,
                        std::span<const unsigned char> plaintext,
                        std::span<const unsigned char> aad,
...
inline std::vector<unsigned char>
aead_decrypt(const secure_vector& key,
             ...
             std::span<const unsigned char> nonce)
{
    ...
    if (1 != EVP_DecryptInit_ex(ctx.get(), EVP_chacha20_poly1305(), nullptr, nullptr,
                                nullptr))
    {
    ...
    if (ctlen > 0)
    {
        if (1 != EVP_DecryptUpdate(ctx.get(), out.data(), &len, ciphertext.data(),
    ...
    if (1 != EVP_DecryptFinal_ex(ctx.get(), out_span.subspan(plen).data(), &len))
    {
                                                ...
                                                std::string_view context)
{
    ...
    if (HMAC(md, zero_salt.data(), static_cast<int>(zero_salt.size()), shared.data(),
             static_cast<int>(shared.size()), prk.data(), &prk_len) == nullptr)
    {
    ...
    secure_vector key(okm.begin(), std::next(okm.begin(), static_cast<std::ptrdiff_t>(okm_len)));
                                               ...
                                               std::uint32_t seq)
{
    ...
    if (HMAC(EVP_sha256(), session_key.data(), static_cast<int>(session_key.size()),
             seqb.data(), seqb.size(), hmac_out.data(), &outlen) == nullptr)
    {
...
inline std::string fingerprint_to_hex(const std::array<unsigned char, SHA256_LEN>& f)
{
...
pqkem_keypair_from_seed(std::string_view alg, const secure_vector& seed)
{
    ...
    if (OQS_KEM_encaps(kem.get(), pk.data(), sk.data(), pk.data()) != OQS_SUCCESS)
    {
