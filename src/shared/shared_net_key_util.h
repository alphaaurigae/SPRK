...
struct SessionId {
     ...
     [[nodiscard]] constexpr auto operator<=>(const SessionId& other) const noexcept = default;
};
...
[[nodiscard]] inline constexpr std::string_view key_load_error_str(KeyLoadError e) noexcept {
    constexpr std::array<std::string_view, 6> msgs = {
        "file not found", "read failed", "parse failed", "invalid key", "no private key", "no public key"
    };
...
inline void load_pem_private_key_async(
     ...
     std::function<void(std::variant<std::pair<secure_vector, secure_vector>, KeyLoadError>)> callback)
{
     asio::post(io, [&io, path, cb = std::move(callback)]() mutable {
         try {
             Poco::File pem_file(path);
             if (!pem_file.exists()) {
                 asio::post(io, [cb = std::move(cb)]() mutable {
                     cb(std::variant<std::pair<secure_vector, secure_vector>, KeyLoadError>{KeyLoadError::FileNotFound});
                 });
                 return;
             }
             ...
             if (!fis.good()) {
                 asio::post(io, [cb = std::move(cb)]() mutable {
                     cb(std::variant<std::pair<secure_vector, secure_vector>, KeyLoadError>{KeyLoadError::ReadFailed});
                 });
                 return;
             }
             ...
             BIO* bio = BIO_new_mem_buf(buffer.begin(), static_cast<int>(bytes_read));
             if (!bio) {
                 asio::post(io, [cb = std::move(cb)]() mutable {
                     cb(std::variant<std::pair<secure_vector, secure_vector>, KeyLoadError>{KeyLoadError::ParseFailed});
                 });
                 return;
             }
             ...
             if (EVP_PKEY_get_raw_private_key(pkey, nullptr, &sk_len) <= 0) {
                 EVP_PKEY_free(pkey);
                 asio::post(io, [cb = std::move(cb)]() mutable {
                     cb(std::variant<std::pair<secure_vector, secure_vector>, KeyLoadError>{KeyLoadError::NoPrivateKey});
                 });
                 return;
             }
             ...
             if (EVP_PKEY_get_raw_public_key(pkey, nullptr, &pk_len) <= 0) {
                 EVP_PKEY_free(pkey);
                 asio::post(io, [cb = std::move(cb)]() mutable {
                     cb(std::variant<std::pair<secure_vector, secure_vector>, KeyLoadError>{KeyLoadError::NoPublicKey});
                 });
                 return;
             }
             ...
             asio::post(io, [cb = std::move(cb), sk = std::move(raw_sk), pk = std::move(raw_pk)]() mutable {
                 std::string pk_hex;
                 pk_hex.reserve(pk.size() * 2);
                 static const char* hx = "0123456789abcdef";
                 for (auto b : pk) { pk_hex.push_back(hx[(b >> 4) & 0xF]); pk_hex.push_back(hx[b & 0xF]); }
                 std::cerr << "[KEYLOAD] async loaded pk_len=" << pk.size() << " pk_prefix=" << pk_hex.substr(0,64) << "\n";
             ...
             asio::post(io, [cb = std::move(cb)]() mutable {
                 cb(std::variant<std::pair<secure_vector, secure_vector>, KeyLoadError>{KeyLoadError::InvalidKey});
             });
         }
     });
}

[[nodiscard]] inline std::variant<std::pair<secure_vector, secure_vector>, KeyLoadError>
load_pem_private_key_sync(const std::string& path) noexcept
{
     try {
         Poco::File pem_file(path);
         if (!pem_file.exists()) {
             return std::variant<std::pair<secure_vector, secure_vector>, KeyLoadError>{KeyLoadError::FileNotFound};
        }
        ...
        if (!fis.good()) {
            return std::variant<std::pair<secure_vector, secure_vector>, KeyLoadError>{KeyLoadError::ReadFailed};
        }
        ...
        BIO* bio = BIO_new_mem_buf(buffer.begin(), static_cast<int>(bytes_read));
        if (!bio) {
            return std::variant<std::pair<secure_vector, secure_vector>, KeyLoadError>{KeyLoadError::ParseFailed};
        }
        ...
        if (EVP_PKEY_get_raw_private_key(pkey, nullptr, &sk_len) <= 0) {
            EVP_PKEY_free(pkey);
            return std::variant<std::pair<secure_vector, secure_vector>, KeyLoadError>{KeyLoadError::NoPrivateKey};
        }
        
        if (EVP_PKEY_get_raw_public_key(pkey, nullptr, &pk_len) <= 0) {
            EVP_PKEY_free(pkey);
            return std::variant<std::pair<secure_vector, secure_vector>, KeyLoadError>{KeyLoadError::NoPublicKey};
        }
...
EVP_PKEY_free(pkey);
{
    ...
    static const char* hx = "0123456789abcdef";
    for (auto b : raw_pk) { pk_hex.push_back(hx[(b >> 4) & 0xF]); pk_hex.push_back(hx[b & 0xF]); }
    std::cerr << "[KEYLOAD] sync loaded pk_len=" << raw_pk.size()
              << " pk_prefix=" << pk_hex.substr(0,64) << "\n";
}
return std::variant<std::pair<secure_vector, secure_vector>, KeyLoadError>{
    std::make_pair(std::move(raw_sk), std::move(raw_pk))
        ...
        return std::variant<std::pair<secure_vector, secure_vector>, KeyLoadError>{KeyLoadError::InvalidKey};
    }
}
...
[[nodiscard]] inline std::optional<std::string> compute_peer_key(const Parsed &p, std::string &peer_fp_hex) noexcept {
