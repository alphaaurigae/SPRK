...
[[nodiscard]] constexpr std::string build_key_derivation_context(std::string_view fp1,
...
[[nodiscard]] constexpr std::string build_username_context(std::string_view username1,
...
public:
    explicit RekeyTimer(asio::io_context& io,
                       std::chrono::milliseconds interval = std::chrono::milliseconds{DEFAULT_MESSAGE_TIMEOUT_MS})
        : timer_{io, interval}
...
class OneShotTimer {
public:
    explicit OneShotTimer(asio::io_context& io)
        : timer_{io}
        , armed_{false}
    {}
    ...
    void start(std::chrono::milliseconds delay, std::function<void(const std::error_code&)> callback) {
        armed_ = true;
        timer_.expires_after(delay);
        timer_.async_wait([this, cb = std::move(callback)](const std::error_code& ec) {
            armed_ = false;
            if (!ec && cb) {
                cb(ec);
            }
...
public:
    explicit RekeyTimeoutManager(asio::io_context& io)
        : io_{io}
    {}
    ...
    [[nodiscard]] std::unique_ptr<RekeyTimer> create_timer(std::chrono::milliseconds interval) {
    ...
    void schedule_rekey(std::chrono::milliseconds delay, std::function<void()> callback) {
        auto timer = std::make_shared<OneShotTimer>(io_);
        
        // Capture timer as shared_ptr to keep it alive until callback completes
        timer->start(delay, [cb = std::move(callback), timer_keepalive = timer](const std::error_code& ec) {
            if (!ec && cb) {
                cb();
            }
            // timer_keepalive goes out of scope here, allowing timer destruction
        });
