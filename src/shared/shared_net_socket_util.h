...
[[nodiscard]] inline int connect_to_host_nonblocking(const char *host,
                                                     int         port) noexcept
{
    const int s = ::socket(AF_INET, SOCK_STREAM, 0);
    if (s < 0)
                        ...
                        std::error_code *out_ec         = nullptr) noexcept
{
    try
    {
        ...
        acceptor->bind(asio::ip::tcp::endpoint(
            asio::ip::tcp::v4(), static_cast<unsigned short>(port)));
...
[[nodiscard]] inline int dup_from_asio_socket(
    const std::shared_ptr<asio::ip::tcp::socket> &sock) noexcept
{
    if (!sock)
        return -1;
...
connect_to_host_asio(asio::io_context &io, const char *host, int port,
                     std::error_code *out_ec = nullptr) noexcept
{
    try
    {
                ...
                asio::async_connect(
                    *sock, endpoints,
                                               ...
                                               const asio::ip::tcp::endpoint &)
                    {
                        ...
                        {
                            int fd_flags =
                                ::fcntl(static_cast<int>(native), F_GETFD);
                            if (fd_flags >= 0)
                                ::fcntl(static_cast<int>(native), F_SETFD,
                                        fd_flags | FD_CLOEXEC);
                        }
