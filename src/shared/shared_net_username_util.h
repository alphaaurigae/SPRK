...
[[nodiscard]] inline int compute_lcs_length(std::string_view a,
...
is_username_too_similar(std::string_view candidate, std::string_view existing,
                        int similarity_threshold_percent = 85) noexcept
{
    ...
    return maxlen > 0 && (100 * lcs) >= (similarity_threshold_percent *
                                         static_cast<int>(maxlen));
}

template <typename MapType>
[[nodiscard]] inline bool
has_similar_username(std::string_view username, const MapType &existing_users,
                     int similarity_threshold = 85) noexcept
{
    return std::ranges::any_of(
        existing_users,
        [username, similarity_threshold](const auto &kv) noexcept {
            return is_username_too_similar(username, kv.first,
...
[[nodiscard]] inline bool is_valid_session_id(std::string_view sid) noexcept
{
    ...
    return sid.size() == SESSION_ID_LENGTH &&
           std::ranges::all_of(
               sid, [BASE58_CHARS](char c) noexcept
...
[[nodiscard]] inline std::string generate_session_id()
{
    ...
    constexpr unsigned int REJECT_THRESHOLD =
        256U / CHARSET_SIZE * CHARSET_SIZE;
