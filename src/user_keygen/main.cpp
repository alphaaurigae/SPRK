...
static EVP_PKEY* generate_ml_dsa_key(const std::string& alg_name) {
    ...
    if (!ctx) throw std::runtime_error("EVP_PKEY_CTX_new_from_name failed for " + alg_name);
...
static void generate_pq_key_and_cert(const std::string& base_name, bool output_raw = false) {
    EVP_PKEY* pkey = generate_ml_dsa_key("ML-DSA-87");

    // 1. PEM format (recommended - standard OpenSSL)
    BIO* bio_mem = BIO_new(BIO_s_mem());
    if (!PEM_write_bio_PrivateKey(bio_mem, pkey, nullptr, nullptr, 0, nullptr, nullptr)) {
        EVP_PKEY_free(pkey);
        BIO_free(bio_mem);
    ...
    X509_NAME_add_entry_by_txt(name, "CN", MBSTRING_ASC,
                               (const unsigned char*)base_name.c_str(), -1, -1, 0);
    ...
    if (X509_sign(x509, pkey, nullptr) <= 0) {
        X509_free(x509);
        EVP_PKEY_free(pkey);
        throw std::runtime_error("X509_sign failed (ML-DSA requires NULL digest)");
    }
    ...
    std::cout << "Generated:\n"
              << "  " << base_name << ".sk.pem   (PEM - recommended for new code)\n";
    if (output_raw) {
